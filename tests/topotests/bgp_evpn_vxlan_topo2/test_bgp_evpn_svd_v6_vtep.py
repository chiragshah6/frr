...
"""
test_bgp_evpn_svd_v6_vtep.py: Test BGP EVPN with VXLAN using Single VXLAN Device (SVD)
...
def test_protocols_convergence():
    ...
    logger.info("Checking BGP convergence on all routers")
    ...
    for rname in ["bordertor-11", "bordertor-12", "leaf-11", "spine-1"]:
        ...
        def check_bgp_summary(router):
            output = router.vtysh_cmd("show bgp summary json", isjson=True)
            if not output:
                return "No BGP summary output"

            # Check if we have neighbors
            if "ipv6Unicast" in output and "peers" in output["ipv6Unicast"]:
                peers = output["ipv6Unicast"]["peers"]
        ...
        test_func = partial(check_bgp_summary, router)
...
def test_remote_vteps_per_vni():
    ...
    for rname in ["bordertor-11", "bordertor-12", "tor-21", "tor-22"]:
        ...
        test_func = partial(
            evpn_verify_remote_vteps, router, vni_list, expected_remote_vteps
        )
...
def test_ipv6_vtep_connectivity():
    ...
    logger.info("Checking IPv6 VTEP reachability on all 4 VTEPs")

    # Check EVPN next-hops use IPv6
    for rname in ["bordertor-11", "bordertor-12", "tor-21", "tor-22"]:
        router = tgen.gears[rname]
...
def test_bgp_sessions():
    ...
    logger.info("Verifying BGP sessions across the topology")

    # Check BGP sessions on various routers including all 4 VTEPs
    routers_to_check = [
        "spine-1",
        "spine-2",
        "leaf-11",
    ...
    for rname in routers_to_check:
        router = tgen.gears[rname]
        output = router.vtysh_cmd("show bgp summary", isjson=False)
