				     ...
				     struct zebra_vxlan_if_update_ctx *ctx)
{
	...
	if (zl3vni) {
		...
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("Update L3-VNI %u intf %s(%u) VLAN %u local IP %pIA master %u chg 0x%x is_l3svd %u",
				   vni, ifp->name, ifp->ifindex, vnip->access_vlan, &vxl->vtep_ip,
				   zif->brslave_info.bridge_ifindex, chgflags, zl3vni->is_l3svd);
				  ...
				  struct zebra_vxlan_vni *vnip)
{
	...
	if (zl3vni) {
		...
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("Add L3-VNI %u intf %s(%u) VLAN %u local IP %pIA master %u vrf %s is_l3svd %u", vni,
				   ifp->name, ifp->ifindex, vnip->access_vlan, &vxl->vtep_ip,
				   zif->brslave_info.bridge_ifindex, vrf_id_to_name(zif->ifp->vrf->vrf_id),
